...
mkdir -p "$root/build"
work=$(mktemp -d "$root/build/retrom-quasi88-web.XXXXXX")
trap 'rm -rf "$work"' EXIT INT TERM
mkdir -p "$work/raw" "$work/build"
cc -D__EMSCRIPTEN__ -I "$root/src/LIBRETRO/libretro-common/include" -fsyntax-only "$root/.github/rpg-runtime/test-web-abi.c"
...
if ! docker run --rm --platform linux/amd64 --hostname retrom-quasi88 \
  ...
  /recipe/build-emulatorjs-core.sh quasi88 \
...
stage="$work/stage"
mkdir -p "$stage"
install -m 0644 "$work/raw/quasi88_libretro.js" "$stage/"
install -m 0644 "$work/raw/quasi88_libretro.wasm" "$stage/"
cat "$root/LICENSE" "$root/src/snddrv/license.txt" "$root/src/fmgen/readme.txt" "$work/raw/retroarch-COPYING" > "$stage/license.txt"
printf '%s\n' '{"minimumEJSVersion":"4.2.2","version":"2.0.2"}' > "$stage/build.json"
printf '%s\n' '{"name":"quasi88","extensions":["d88","u88","m3u"],"makeoptions":{"buildpath":"./","makescript":"Makefile","arguments":[]},"options":{},"save":false,"license":"LICENSE","repo":"https://github.com/retrom-project/quasi88-libretro"}' > "$stage/core.json"

(cd "$stage" && 7z a -mtm=off -mta=off -mtc=off -bd -bso0 -bsp0 -t7z "$output/quasi88-wasm.data" \
  quasi88_libretro.js quasi88_libretro.wasm build.json core.json license.txt)
